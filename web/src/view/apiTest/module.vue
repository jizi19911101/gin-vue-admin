<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="模块名称" prop="name" width="300" />
        <!-- <el-table-column align="left" label="日期" width="180">
            <template #default="scope">{{ formatDate(scope.row.CreatedAt) }}</template>
        </el-table-column> -->
        <el-table-column align="left" label="操作">
            <template #default="scope">
            <el-button type="text" icon="el-icon-files" size="small" class="table-button" @click="goToApiList(scope.row)">查看接口</el-button>
...
export default {
  ...
  // 条件搜索前端看此方法
    ...
    goToApiList(item){
      this.$router.push({name:"apiList", query:{module:item.name}})
    },
    goToReport(){
